...
class StudentController extends Controller
{
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
                    ...
                    $model->save();
					return $this->redirect(['update', 'id' => $model->student_id]);
    ...
    public function actionAddToBatch()
	{
		if (isset($_POST['keys'])) {
    ...
    public function actionStat()
    {
				...
				//~ $male = Student::model()->findByAttributes(array('sex'=>"male"));
				//~ $female = Student::model()->findByAttributes(array('sex'=>"female",'record_status'=>"Active"));
				
				//~ $males=Student::find(array('condition'=>'sex =:sex','params'=>array(':sex'=>'male'))); 
				//~ $females=Student::find(array('condition'=>'sex =:sex','params'=>array(':sex'=>'female'))); 
				//~ $count = Student::model()->countByAttributes(array(
					//~ 'sex'=> 'female'
				//~ ));
				$male_num = Student::find()->where(['sex' => 'male','record_status'=>"Active"])->count();
				$female_num = Student::find()->where(['sex' => 'female','record_status'=>"Active"])->count();
				$total = $male_num + $female_num;
					
					return [
						'male' => $male_num/$total*100,
						'female' => $female_num/$total*100,
