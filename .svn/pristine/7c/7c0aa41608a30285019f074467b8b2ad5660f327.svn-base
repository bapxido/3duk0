...
<div class="class-subject-header-form">
    ...
    <?php $form = ActiveForm::begin(['options' => ['enctype' => 'multipart/form-data','id'=>'class-subject-header-form']]);?>
...
-->
	...
	<?php
	    ...
	    $lesson_plan = $lesson_plan .  \yii2fullcalendar\yii2fullcalendar::widget([
	    	'ajaxEvents' => Url::to(['/class-lesson-plan/index'])
	    	]); 
    ?>
	...
	<?php    
		$items = [
			 [
	            'label'=>'<i class="fa fa-glyphicon-blackboard"></i> Class Subject',
	            'content'=>$this->render('form_fields', ['model' => $model, 'form' => $form]),
	            'encode'=>false,
	        ],
	        [
	            'label'=>'<i class="fa fa-list-alt"></i> Class Subject Roll',
	            'content'=>$ClassSubjectRoll,
	            'encode'=>false,
	        ],
	        [
	            'label'=>'<i class="fa fa-th-list"></i> Subject Notes',
	            'content'=>$SubjectNotes,
	            'encode'=>false,
	        ],
	        [
				 'label'=>'<i class="fa fa-briefcase"></i> Subject Assignments',
				 'encode'=>false,
				 'content'=>$SubjectAssignments,
	        ],
	        [
				 'label'=>'<i class="fa fa-book"></i> Class Tests',
				 'encode'=>false,
				 'content'=>$ClassTests,
	        ],
	        [
				 'label'=>'<i class="fa fa-calendar"></i> Lesson Planner',
				 'encode'=>false,
				 'content'=>$lesson_plan,
	        ]
	    ];
